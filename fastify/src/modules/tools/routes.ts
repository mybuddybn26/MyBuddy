...
type Pending = Record<
  string,
  { tool: string; params: Record<string, unknown>; userId: string }
>;
...
export default fp(async (app: FastifyInstance) => {
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!pending)
        return reply.status(404).send({ detail: 'Confirmation not found' });

      const tool = getTool(pending.tool);
      if (!tool)
        return reply
          .status(400)
          .send({ detail: `Unknown tool: ${pending.tool}` });

      const result = await tool.handler(app.db, pending.userId, pending.params);
      delete confirmations[confirmationId];

      if (result.ok)
        return reply.send({ ok: true, tool: pending.tool, data: result.data });
