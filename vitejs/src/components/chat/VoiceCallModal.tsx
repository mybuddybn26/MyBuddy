...
export function VoiceCallModal({ open, onClose }: VoiceCallModalProps) {
  ...
  if (!isCallActive(state)) {
    return (
      ...
      >
        <div
          className='bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full mx-4 animate-slide-up text-center'
        ...
        >
          ...
          <h2 className='text-xl font-bold text-slate-800 mb-1'>
            Voice Call with Buddy
          </h2>
          <p className='text-sm text-slate-500 mb-6'>
            Talk naturally — Buddy listens and responds like a real
            conversation.
          </p>
          {error && (
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`w-24 h-24 rounded-full bg-primary-100 flex items-center justify-center transition-all duration-500 ${state === 'speaking' ? 'scale-110 shadow-lg shadow-primary-200 animate-pulse' : ''}`}
        >
