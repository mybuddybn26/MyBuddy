...
export function Chat() {
  ...
  useEffect(() => {
    api
      ...
      .catch(() => ({ data: [], count: 0 }))
      .then((chatRes) => {
        olderOffsetRef.current = (
          chatRes.data as Array<Record<string, unknown>>
        ).length;
  ...
  const sendMessage = useCallback(
    async (text: string, inputType = 'text') => {
  ...
  const onVoiceBubble = useCallback(
    (phase: string, role: 'user' | 'assistant', content: string) => {
      ...
      if (phase === 'transcribing' && role === 'user') {
        ...
        setMessages((prev) => [
          ...prev,
          { id, role: 'user', content: 'Transcribing...', streaming: true },
        ]);
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === b.assistantId ? { ...m, content, streaming: false } : m,
          ),
        );
        ...
        if (b.assistantId) {
          setMessages((prev) => prev.filter((m) => m.id !== b.assistantId));
          b.assistantId = undefined;
        }
      }
    },
  ...
  const onVoiceReveal = useCallback((text: string) => {
    ...
    if (b.assistantId) {
      setMessages((prev) =>
        prev.map((m) => (m.id === b.assistantId ? { ...m, content: text } : m)),
      );
    }
  }, []);
  ...
  const handleToolConfirm = useCallback(
    async (msgId: string, confirmationId: string) => {
      try {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === msgId ? { ...m, toolConfirm: undefined } : m,
          ),
        );
      }
    },
  ...
  return (
    <div className='flex flex-col h-full'>
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            >
              ...
              {msg.budgets &&
                msg.budgets.length > 0 &&
                msg.budgets.some((b) => b.items && b.items.length > 0) && (
                    ...
                    <div className='flex gap-2'>
                      <button
                        onClick={() =>
                          handleToolConfirm(
                            msg.id,
                            msg.toolConfirm!.confirmationId,
                          )
                        }
                        className='flex-1 py-1.5 bg-primary-500 text-white rounded-lg text-xs font-medium hover:bg-primary-600 transition-colors'
                      >
                        Confirm
                      </button>
                      <button
                        ...
                        className='flex-1 py-1.5 border border-slate-200 text-slate-500 rounded-lg text-xs font-medium hover:bg-slate-50 transition-colors'
                      >
                        Cancel
                      </button>
                    </div>
      ...
      <VoiceCallPanel
        ...
        onBubble={onVoiceBubble}
        onRevealText={onVoiceReveal}
      />
    </div>
  );
}
