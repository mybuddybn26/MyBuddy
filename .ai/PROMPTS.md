...
- Dialect: Brunei Malay expressions when `persona.dialect === 'brunei'` (bisai, banar, apa khabar).
- Contractions: Uses I'm, you're, it's, we'll, I'd, that's, here's, there's naturally.
...
```typescript
// documents/routes.ts
const result = await analyzeImage(imageBase64, mediaType, documentAnalysisPrompt, persona);
```
